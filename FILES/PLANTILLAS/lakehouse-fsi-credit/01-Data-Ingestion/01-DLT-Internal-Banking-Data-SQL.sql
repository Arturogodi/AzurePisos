...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=3987092476296021&notebook=%2F01-Data-Ingestion%2F01-DLT-Internal-Banking-Data-SQL&demo_name=lakehouse-fsi-credit&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-fsi-credit%2F01-Data-Ingestion%2F01-DLT-Internal-Banking-Data-SQL&version=1">
...
CREATE OR REFRESH STREAMING LIVE TABLE credit_bureau_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/credit_bureau', 'json',
                 map('header', 'true', 
                     'inferSchema', 'true', 
...
CREATE OR REFRESH STREAMING LIVE TABLE customer_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/internalbanking/customer', 'csv',
                 map('header', 'true', 
                     'inferSchema', 'true', 
                     'cloudFiles.inferColumnTypes', 'true',
...
CREATE OR REFRESH STREAMING LIVE TABLE relationship_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/internalbanking/relationship', 'csv',
                 map('header', 'true', 
                     'inferSchema', 'true', 
                     'cloudFiles.inferColumnTypes', 'true'))
...
CREATE OR REFRESH STREAMING LIVE TABLE account_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/internalbanking/account', 'csv',
                 map('header', 'true', 
                     'inferSchema', 'true', 
                     'cloudFiles.inferColumnTypes', 'true'))
...
CREATE OR REFRESH STREAMING LIVE TABLE fund_trans_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/fund_trans', 'json',
...
CREATE OR REFRESH STREAMING LIVE TABLE telco_bronze AS
  SELECT * FROM
    cloud_files('/Volumes/main/dbdemos_fsi_credit_decisioning/credit_raw_data/telco', 'json',
                 map('inferSchema', 'true',
                     'cloudFiles.inferColumnTypes', 'true'))
