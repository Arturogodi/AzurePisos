...
# DBTITLE 0,--i18n-3fb78239-dd9c-4c5d-b4dc-6049604db4b2
# MAGIC %md
# MAGIC
# MAGIC
# MAGIC # Classes
...
# DBTITLE 0,--i18n-f8f3096e-b29e-4c8e-8874-96451f47002a
# MAGIC %md
# MAGIC
# MAGIC
# MAGIC ## Classes
...
# DBTITLE 0,--i18n-b326ce06-9f66-46f3-a134-03579d30867a
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-58c970b2-4bd1-4b0b-8533-b0794b07eed3
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-5de3ea33-9f20-4ca7-913c-f08dba3ba9ca
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-1dd48eff-d93e-4f8e-9373-ae10de48af4d
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-56944397-f05e-4cdd-bced-940be45e1872
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-a3aea935-bc40-4fc2-aeb0-f3d5d7c45eb2
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-a6ca39f1-ceb0-42ff-aa95-fac34ef7ba1b
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-3d8f5672-e9d4-4bd4-9c14-91756c1cbb4f
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-4ec0fb49-f058-4c68-be13-aac66e9e5fe3
# MAGIC %md
# MAGIC
# MAGIC
...
# DBTITLE 0,--i18n-f781f535-f48d-4fb6-a110-c52e5a6c4039
# MAGIC %md
# MAGIC
# MAGIC
...
class CustomerLoanPropensity:
    def __init__(self, customer_data):
        """
        Inicializa la clase con los datos del cliente.
        :param customer_data: DataFrame con los datos del cliente.
        """
        self.customer_data = customer_data
    
    def calculate_credit_score(self):
        """
        Método que calcula un puntaje de crédito basado en historial de pagos y saldos.
        """
        self.customer_data['credit_score'] = self.customer_data['payment_history'] * 0.6 + self.customer_data['balance'] * 0.4
        return self.customer_data[['customer_id', 'credit_score']]
    
    def calculate_loan_propensity(self):
        """
        Método que calcula la probabilidad de que el cliente solicite un préstamo en base a múltiples factores.
        """
        # Ejemplo de fórmula de propensión a préstamo (simplificada)
        self.customer_data['loan_propensity'] = (self.customer_data['transaction_volume'] * 0.5 +
                                                 self.customer_data['account_age'] * 0.3 +
                                                 self.customer_data['credit_score'] * 0.2)
        return self.customer_data[['customer_id', 'loan_propensity']]

    def categorize_customers(self):
        """
        Método que categoriza a los clientes en función de su propensión a solicitar un préstamo.
        """
        def categorize(propensity):
            if propensity > 0.75:
                return 'Alta'
            elif propensity > 0.5:
                return 'Media'
        ...
        self.customer_data['loan_category'] = self.customer_data['loan_propensity'].apply(categorize)
        return self.customer_data[['customer_id', 'loan_category']]

# Supongamos que tenemos un DataFrame con los datos de los clientes
# customer_data_df = spark.read.csv("path_to_bank_customer_data")

# Instanciamos la clase con los datos del cliente
propensity_model = CustomerLoanPropensity(customer_data_df)

# Calcular puntaje de crédito
credit_scores_df = propensity_model.calculate_credit_score()

# Calcular propensión a préstamos
loan_propensity_df = propensity_model.calculate_loan_propensity()

# Categorizar clientes por su propensión a préstamos
categorized_customers_df = propensity_model.categorize_customers()

# Mostrar el resultado final
categorized_customers_df.show()
